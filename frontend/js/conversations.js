(async function () {
  ...
  async function init() {
    ...
    if (scroll) {
      scroll.innerHTML = `
        <div class="skeleton" style="height:60px;margin-bottom:12px;border-radius:10px"></div>
        <div class="skeleton" style="height:60px;margin-bottom:12px;border-radius:10px"></div>
        <div class="skeleton" style="height:60px;margin-bottom:12px;border-radius:10px"></div>
      `;
    }
    try {
      ...
      toast(e.message);
      if (scroll) {
        scroll.innerHTML = `<div style="padding:24px 16px; text-align:center; color:var(--neg); font-size:13px;">
          Failed to load conversations: ${esc(e.message)}
          <button class="btn btn-sm btn-primary d-block mx-auto mt-2" id="retry-init">Retry</button>
        </div>`;
        const btn = document.getElementById("retry-init");
